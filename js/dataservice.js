const productsList = async () => {
    try {
        ...
        if (!response.ok) {
            throw new Error('Não foi possível ter a lista de produtos');
        }
        return await response.json();
    } catch (error) {
        console.error('Erro ao ter a lista de produtos:', error.message);
        throw error;
    }
};
...
const createNewElement = async (name, value, image) => {
    try {
        ...
        if (!response.ok) {
            throw new Error('Não foi possivel criar um novo produto');
        }
        return await response.json();
    } catch (error) {
